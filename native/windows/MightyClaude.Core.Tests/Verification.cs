...
internal static class Verification
{
    ...
    internal static async Task RunAsync()
    {
        ...
        await Test("model defaults explicit model wins over mode default", ModelDefaultsVerification.ExplicitModelWinsOverModeDefault);
        await Test("model defaults explicit model ignores workspace and app defaults", ModelDefaultsVerification.ExplicitModelIgnoresWorkspaceAndAppDefaults);
        await Test("model defaults default session uses app mode default", ModelDefaultsVerification.DefaultSessionUsesAppModeDefault);
        await Test("model defaults no defaults return default", ModelDefaultsVerification.NoDefaultsReturnDefault);
        await Test("model defaults workspace override beats app default", ModelDefaultsVerification.WorkspaceOverrideBeatsAppDefault);
        await Test("model defaults workspace default value falls through to app default", ModelDefaultsVerification.WorkspaceDefaultValueFallsThroughToAppDefault);
        await Test("model defaults workspace override only applies to its mode", ModelDefaultsVerification.WorkspaceOverrideOnlyAppliesToItsMode);
        await Test("model defaults mode menu label is resolved for mode", ModelDefaultsVerification.ModeMenuLabelIsResolvedForMode);
        await Test("model defaults mode menu label respects workspace over app", ModelDefaultsVerification.ModeMenuLabelRespectsWorkspaceOverApp);
        await Test("model defaults explicit selection survives permission mode changes", ModelDefaultsVerification.ExplicitSelectionSurvivesPermissionModeChanges);
        await Test("model defaults codex provider uses codex defaults", ModelDefaultsVerification.CodexProviderUsesCodexDefaults);
        await Test("model defaults AppSnapshot ModelDefaults round-trips", ModelDefaultsVerification.AppSnapshotModelDefaultsRoundTrips);
        await Test("model defaults old snapshot without field decodes as null version stays 1", ModelDefaultsVerification.OldSnapshotWithoutModelDefaultsDecodesAsNullVersionStays1);
