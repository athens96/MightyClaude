...
internal static class StatusLineVerification
{
    ...
    internal static Task ConfigFollowsPrecedenceAndOnlyCommandEntries()
    {
        ...
        try
        {
            ...
            var user = StatusLineSupport.Discover(null, home);
            ...
            var proj = StatusLineSupport.Discover(workspace, home);
            ...
            var local = StatusLineSupport.Discover(workspace, home);
            ...
            var noCmd = StatusLineSupport.Discover(null, home);
            ...
            var envOverride = StatusLineSupport.Discover(null, home,
                new Dictionary<string, string> { ["CLAUDE_CONFIG_DIR"] = altConfig });
    ...
    internal static Task TrustRuleRequiresFingerprintForWorkspaceCommands()
    {
        ...
        Check(StatusLineTrust.Resolve(ws, snapshot, workspaceId) == (null, ws), "an untrusted workspace command is shown as a question, not run");
        Check(StatusLineTrust.Resolve(user, snapshot, workspaceId) == (user, null), "a user-level command runs without a prompt");
        ...
        Check(StatusLineTrust.Resolve(ws, snapshot, workspaceId) == (ws, null), "once allowed the workspace command runs and the question disappears");
        Check(StatusLineTrust.Resolve(ws2, snapshot, workspaceId) == (null, ws2), "a changed command asks again");
