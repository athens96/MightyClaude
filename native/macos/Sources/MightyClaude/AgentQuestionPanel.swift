...
struct AgentQuestionPanel: View {
    ...
    var body: some View {
        let progress = store.ouroborosProgress(for: sessionId, request: request)
        ...
        return VStack(alignment: .leading, spacing: 7) {
            if let current = progress.current(in: questionnaire) {
                HStack(spacing: 6) {
                    ...
                    Spacer()
                    if progress.index > 0 { Button("이전") { store.ouroborosBack(sessionId) }.controlSize(.mini).disabled(busy) }
                ...
                VStack(alignment: .leading, spacing: 4) {
                    ForEach(current.options, id: \.label) { option in
                        ...
                        Button { onPrepare(); store.ouroborosChoose(sessionId, option: option.label) } label: {
                ...
                HStack(spacing: 6) {
                    ...
                    if current.multiSelect, !progress.selected.isEmpty { Button("선택 완료") { onPrepare(); store.ouroborosAnswer(sessionId, text: "") }.controlSize(.mini).disabled(busy) }
