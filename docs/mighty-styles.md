...
> **유일한 예외: 도구 모음(toolkit).** 설정 → 구성 요소의 "내 작업 도구 모음"은 사용자가 확인 화면에서 직접 승인한 뒤 명령을 argv 배열로 직접 실행한다. 터미널 창을 쓰지 않으므로 이 `autoRun: false` 규칙이 적용되지 않는다. 자세한 내용은 [docs/toolkit.md](toolkit.md)를 참고한다.
...
- 어떤 문자열도 **자동으로 실행되지 않는다**. 설치 명령은 터미널 창에 `autoRun: false`로 채워질 뿐이고, 붙여넣기 직전 줄바꿈 검사를 한 번 더 받는다(1.5). 단, 도구 모음(toolkit)은 예외로 사용자가 확인 화면에서 승인한 뒤 직접 실행된다([docs/toolkit.md](toolkit.md)).
