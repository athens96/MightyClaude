...
> 과거 구현 기록: 2026-09-21 입력창 재설계에서 직접 한글 조합과 자동 입력 컨텍스트 복구를 메시지 입력 경로에서 제거했습니다. 현재 구조는 `docs/input-method-diagnostics.md`를 참고하세요. 아래 코어 조합기와 검사는 과거 동작을 설명하기 위해 남겨 둡니다.
