...
- **리셋권(한도 리셋 자격) 표시**: `native/macos/Sources/MightyClaude/StatusBarUsage.swift`, `native/macos/Sources/MightyCore/AccountUsageService.swift`, `native/macos/Sources/MightyCore/AccountUsageSnapshot.swift`, `native/macos/Tests/MightyCoreTests/AccountUsageTests.swift`, `native/windows/MightyClaude.Core/AccountUsage.cs`, `native/windows/MightyClaude.Core.Tests/AccountUsageVerification.cs`, `native/windows/MightyClaude.WinUI/MainWindow.Smoke.cs`, `locales/ko.json`, `locales/en.json` — v5 두 번째 항목. `scripts/check-style-freeze.sh`가 이 경로들을 포함해 exit 1로 끝나는 것이 v5 태그 전 정상 상태이다("freeze check passes"는 완료 조건이 아님).
