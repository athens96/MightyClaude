...
`docs/i18n.md`, `docs/styles-followups.md`, `native/windows/MightyClaude.Core.Tests/SettingsSectionsVerification.cs`, `native/windows/MightyClaude.Core/SettingsSections.cs`, `native/windows/MightyClaude.Core/ToolkitStore.cs`, `native/windows/MightyClaude.WinUI/MainWindow.Settings.cs` — v5 열한 번째 항목. Components 칸을 Windows에 등록하고, CLI 줄·도구 목록·설치 흐름을 WinUI에 구현했다. 이 경로들도 v5 태그 전까지 `scripts/check-style-freeze.sh`의 exit 1에 포함되는 것이 정상이다.
